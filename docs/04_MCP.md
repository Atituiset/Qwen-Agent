...
### 4.2 初始化流程（initConfig → init_config_async）

```python
def init_config_async(self, config: Dict) -> list:
    tools = []
    for server_name, server_cfg in config.items():
        # 1. 创建 MCPClient 并连接
        client = MCPClient()
        await client.connection_server(server_name, server_cfg)
        self.clients[server_name] = client

        # 2. 获取 Server 提供的工具列表
        mcp_tools = await client.list_tools()

        # 3. 为每个工具创建动态 BaseTool
        for tool in mcp_tools:
            mcp_tool = self.create_tool_class(
                register_name=tool.name,
                register_client_id=server_name,
                tool_name=tool.name,
        ...
        # 4. 如果 Server 暴露了 resources，额外创建两个工具
